...
# Register the hash function as a UDF
hash_udf = udf(hash_value, StringType())

# Apply the hashing UDF to the PII columns: FirstName, LastName, Email
hashed_df = df.withColumn("FirstName", hash_udf(df["FirstName"]))\
              .withColumn("LastName", hash_udf(df["LastName"]))\
              .withColumn("Email", hash_udf(df["Email"]))

# Convert DataFrame to DynamicFrame
cleaned_dynamic_df = DynamicFrame.fromDF(
    hashed_df, glueContext, "cleaned_dynamic_df")
